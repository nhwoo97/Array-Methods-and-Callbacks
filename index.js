...
/*let final2014= findIndex(
    array =>array.Year === 2014 && array.Stage === 'Final'
);
...
function getFinals(data) {
    let finalsOnly =  fifaData.filter(function(item){
...
function getWinners(cb, data) {
    ...
    cb(data).forEach(function(item){
        if (item['Home Team Goals']> item['Away Team Goals'])(winners.push['Home Team Name']);
...
function getWinnersByYear(winners, years, data) {
    ...
    const allWinners = [];
    getFinals(data).forEach(()item, i) => {allWinners.push(`In $(year[i]), $(country[i]) won the world cup!`);
    return allWinners; 
})
};
...
function getAverageGoals(data) {
    ...
    const totalGoals= data.reduce(total, match)=>(
        return {
            total + match['Home Team Goals']+atch['Away Team Goals']
        };
},0)
